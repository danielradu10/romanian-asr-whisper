...
ROMANIAN_STOPWORDS = {
    ...
    # demonstrative / filler words that polluted the first clustering
    "acest", "aceasta", "aceste", "acestea", "acela", "aceea",
    "acolo", "aici", "asta", "ăsta", "ăștia", "acești",
    "lucru", "lucruri", "tot", "toate", "toți", "toate",
}
...
def coalesce_column(df: pd.DataFrame, target: str, candidates: list[str], default: Any) -> pd.DataFrame:
...
def analyze_errors(
    ...
    df = coalesce_column(
        df=df,
        target="audio_cluster",
        candidates=["cluster", "cluster_metadata"],
        default="unknown",
    )
    ...
    df = coalesce_column(
        ...
        candidates=["duration_seconds", "duration_seconds_metadata"],
        default=0.0,
    )
    ...
    df = coalesce_column(
        df=df,
        target="split",
        candidates=["split", "split_metadata"],
        default="unknown",
    )

    if "text_cluster_stopwords" not in df.columns:
        raise ValueError("text_cluster_stopwords was not produced or merged correctly.")
    ...
    overall = pd.DataFrame([
        {
            ...
            "text_clustering_variant": "tfidf_romanian_stopwords",
            "n_text_clusters": n_text_clusters,
            "min_df": min_df,
            "max_df": max_df,
            "max_features": max_features,
    ...
    summarize_group(errors_df, "text_cluster_stopwords").to_csv(
    ...
    save_cluster_examples(
        errors_df=errors_df,
        output_path=output_dir / "worst_samples_by_text_cluster.csv",
        cluster_column="text_cluster_stopwords",
    ...
    print()
    print("Error summary by text cluster with Romanian stopwords:")
    print(summarize_group(errors_df, "text_cluster_stopwords").to_string(index=False))

    print()
    print("Top terms per text cluster with Romanian stopwords:")
    print(top_terms_df.to_string(index=False))

    print()
    print(f"Saved stopword-based error analysis to: {output_dir}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/error_analysis_stopwords"),
    )
    parser.add_argument(
        "--n-text-clusters",
        type=int,
